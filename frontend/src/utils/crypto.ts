...
export async function getOrCreateKeys(): Promise<KeyPair> {
  ...
  try {
    ...
    try {
      ...
      const keyPair = (await window.crypto.subtle.generateKey({ name: 'X25519' }, true, [
        'deriveKey',
        'deriveBits',
      ])) as CryptoKeyPair;
    ...
    if (publicKey.length === 0 || privateKey.length === 0) {
      throw new Error(
        `Invalid key length: public=${publicKey.length}, private=${privateKey.length}`,
      );
    }
